...
//Pages Table
define('SOCIALWIKI_TABLE_ALL_PAGES', 1);
//Users Tables
define('SOCIALWIKI_TABLE_ALL_USERS', 2);
//Version Tables
define('SOCIALWIKI_TABLE_RECOMENDED_VERSIONS', 3);
define('SOCIALWIKI_TABLE_NEW_VERSIONS', 4);
define('SOCIALWIKI_TABLE_USER_CREATED_VERSIONS', 5);
...
define('SOCIALWIKI_COLUMN_VERSION_UPDATED', 3);
define('SOCIALWIKI_COLUMN_VERSION_LIKES', 4);
define('SOCIALWIKI_COLUMN_VERSION_FAVORITE', 5);
define('SOCIALWIKI_COLUMN_VERSION_AUTHOR_POP_MAX', 6);
define('SOCIALWIKI_COLUMN_VERSION_LIKE_SIM_MAX', 7);
define('SOCIALWIKI_COLUMN_VERSION_FOLLOW_SIM_MAX', 8);
define('SOCIALWIKI_COLUMN_VERSION_NETWORK_DISTANCE_MAX', 9);
define('SOCIALWIKI_COLUMN_VERSION_AUTHOR_POP_MIN', 10);
define('SOCIALWIKI_COLUMN_VERSION_LIKE_SIM_MIN', 11);
define('SOCIALWIKI_COLUMN_VERSION_FOLLOW_SIM_MIN', 12);
define('SOCIALWIKI_COLUMN_VERSION_NETWORK_DISTANCE_MIN', 13);
define('SOCIALWIKI_COLUMN_VERSION_AUTHOR_POP_AVG', 14);
define('SOCIALWIKI_COLUMN_VERSION_LIKE_SIM_AVG', 15);
define('SOCIALWIKI_COLUMN_VERSION_FOLLOW_SIM_AVG', 16);
define('SOCIALWIKI_COLUMN_VERSION_NETWORK_DISTANCE_AVG', 17);
define('SOCIALWIKI_COLUMN_VERSION_AUTHOR_POP_SUM', 18);
define('SOCIALWIKI_COLUMN_VERSION_LIKE_SIM_SUM', 19);
define('SOCIALWIKI_COLUMN_VERSION_FOLLOW_SIM_SUM', 20);
define('SOCIALWIKI_COLUMN_VERSION_NETWORK_DISTANCE_SUM', 21);
define('SOCIALWIKI_COLUMN_VERSION_VIEWS', 31);

define('SOCIALWIKI_COLUMN_USER_NAME', 22);
define('SOCIALWIKI_COLUMN_USER_FOLLOWERS', 23);
define('SOCIALWIKI_COLUMN_USER_SOCIAL_DISTANCE', 24);
define('SOCIALWIKI_COLUMN_USER_LIKE_SIM', 25);
define('SOCIALWIKI_COLUMN_USER_FOLLOW_SIM', 26);

define('SOCIALWIKI_COLUMN_PAGE_TITLE', 27);
define('SOCIALWIKI_COLUMN_PAGE_VIEWS', 28);
...
function socialwiki_set_default_tables($user_id) {
    ...
    foreach ($tables as $table => $enabled) {
        ...
        $insertid = $DB->insert_record('socialwiki_table', $record);
    }
...
function socialwiki_get_all_table_columns() {
    $tables = array(
        ...
        SOCIALWIKI_TABLE_ALL_USERS => array(
            SOCIALWIKI_COLUMN_USER_NAME => ENABLE,
            SOCIALWIKI_COLUMN_USER_FOLLOWERS => ENABLE,
            SOCIALWIKI_COLUMN_USER_SOCIAL_DISTANCE => ENABLE,
            SOCIALWIKI_COLUMN_USER_LIKE_SIM => ENABLE,
            SOCIALWIKI_COLUMN_USER_FOLLOW_SIM => ENABLE,
        ),
        SOCIALWIKI_TABLE_ALL_PAGES => array(
            SOCIALWIKI_COLUMN_PAGE_TITLE => ENABLE,
            SOCIALWIKI_COLUMN_PAGE_VIEWS => ENABLE,
            SOCIALWIKI_COLUMN_PAGE_LIKES => ENABLE,
...
function socialwiki_get_default_columns($user_id) {
    ...
    foreach ($tables as $table => $columns) {
        foreach($columns as $column => $enabled) {
            ...
            $insertid = $DB->insert_record('socialwiki_column', $record);
        }
    }
...
function socialwiki_has_table_defaults($user_id)
{
    Global $DB;
    $sql='SELECT table_id 
          FROM {socialwiki_table}
          WHERE user_id=?';
          
    return $DB->record_exists_sql($sql,array($user_id));
}

function socialwiki_has_column_defaults($user_id)
{
    Global $DB;
    $sql='SELECT table_id 
          FROM {socialwiki_column}
          WHERE user_id=?';
          
    return $DB->record_exists_sql($sql,array($user_id));
}

function socialwiki_table_is_enabled($user_id, $table_id)
{
    global $DB;
    return $DB->get_record('socialwiki_table', 
